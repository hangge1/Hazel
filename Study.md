...
### P50、51、52、53、54  【2D渲染器开始、变换、纹理、独立着色器】



独立着色器实现既可以颜色显示，也可以纹理显示，也可以混合的原理。
...
实际的最后颜色表示为：`color = texture(tex, texCoor) * u_Color`

所以，当我们如果要利用颜色显示的时候，需要满足：
...
知道每个函数，每帧花了多少ms，就知道哪里需要优化！

但是光知道ms还不够直观，我们需要可视化的直观手段！
...
VS也自带分析工具！但是拥有自己开发的性能分析工具，更加有意义，他可以跨平台，防止引入各种无意义不同平台工具



### P56 性能分析可视化

通过按照一定的json格式写入文件，然后通过浏览器的chrome://tracing/加载显示



### P57 仪表

封装相应的仪表类和定时器仪表类，用于更好的使用



### P58、59、60、61、62、63、64 
...
提供当前每一帧调用多少次DrawCall、多少个四边形、多少个顶点等
